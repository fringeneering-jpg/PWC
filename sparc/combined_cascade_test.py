...
if __name__ == "__main__":
    ...
    for seed in range(10):
        ...
        # simple single-c cascade on same split, for direct comparison
        obj1 = lambda c: ct.galaxy_balanced_rms(gal_data, train, c)
        from scipy.optimize import minimize_scalar
        res1 = minimize_scalar(obj1, bounds=(0.01,5.0), method="bounded", options={"xatol":1e-3})
        ...
        print(f"seed={seed}: c={c_fit:.3f} a0={a0_fit:.3e}  train={rms_train:.4f}  "
              f"holdout_combined={rms_hold:.4f}  holdout_simple_cascade={rms_hold_simple:.4f}")
    ...
    print(f"combined beats simple cascade in {sum(h1<h2 for h1,h2 in zip(hc,hs))}/10 seeds")
