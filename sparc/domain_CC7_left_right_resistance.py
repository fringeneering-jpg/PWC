"""
DOMAIN CC7 -- the literal left/right (inward/outward) resistance ratio.
Confirmed geometry: left = inward (toward galactic centre), right = outward.
Medium relaxes almost instantly, no multi-orbit accumulation -- this is a
purely local, instantaneous comparison at each radius, repeated identically
every orbit: which side has more resistance (mass), and you drift that way.

  M_in(R)  = enclosed mass inside R   (real, from g_bar(R)*R^2/G)
  M_out(R) = mass remaining outside R, out to the galaxy's own last measured
             point (M_tot - M_in)
  tension  = log10(M_in / M_out)      -- UNBOUNDED, grows large near the
             edge (M_out -> 0) instead of saturating like the earlier
             bounded (2*M_in/M_tot - 1) version did

  g_pred = choke(g_bar, a0, n=1/2) * (1 + lam*tension)

Prediction: lam > 0 (more inward-dominant ratio -> extra inward pull).
...
for gname in set(name):
    ...
    if Mtot <= 0 or not np.isfinite(Mtot):
        tension[order] = np.nan
        continue
    Mout_g = np.maximum(Mtot - Menc_g, 1e-4*Mtot)   # floor to avoid log(0) at the very edge
    Min_g  = np.maximum(Menc_g, 1e-4*Mtot)
    tension[order] = np.log10(Min_g/Mout_g)
valid = np.isfinite(tension)
print(f"usable points: {len(g_obs)}, valid tension: {valid.sum()}")
print(f"tension (log M_in/M_out) range: {np.nanmin(tension):.2f} to {np.nanmax(tension):.2f}, median {np.nanmedian(tension):.2f}")
...
json.dump(dict(stability=stability, n_wins=n_wins, n_seeds=len(SEEDS),
          ...
          open(f"{D}/domain_CC7_results.json","w"), indent=2)
