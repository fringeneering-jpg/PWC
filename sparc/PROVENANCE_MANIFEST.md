...
## External document extraction (2026-09-21)

User-directed full read (no skipping) of 8 external source documents accumulated over prior days of independent work with other AI systems, processed via five parallel sub-agent reads and folded into `provenance_manifest.json`'s existing structure (`excluded_substitutions`, `surrogate_and_numerical_audits.domains`, `current_derivations`, `external_citations_2026-09-21`, `external_document_extraction_2026-09-21`). Full detail lives in the JSON; the highest-value items:

- **Domains CC-HH, a real falsification chain not previously recorded in this manifest.** Committed in this project's own git history (PR #1, 6 commits, `c4aa8aa`), found in `CONVERSATION_TRANSCRIPT (1).md`. A universal HDF equation of state was tested with a gauge-free, zero-free-parameter method (Domain EE) and rejected: stiffness scatters a factor of 5 at fixed density, and tracks each galaxy's own V_flat at correlation +0.921 -- structurally a halo fit in fluid vocabulary. Forcing rigid co-rotation (Domain FF) doesn't break this, it moves it into a cosmological spin-parameter requirement 11.3x too high (4.8-sigma, 0/124 galaxies within 2-sigma). Two rescue attempts (Domain GG: oblateness artifact, cosmological spin) were both independently rejected.
- **A rigorous rejection of "the medium is literally the electromagnetic field."** Four independent, quantified failures: 1,700x-22,000x too stiff (traceless EM stress tensor forces c_s=c/sqrt(3)); a Jeans length 2,000x-600,000x too large to let matter clump (the Mészáros effect); a required surface tension 2.6 million times water's value with no intrinsic length scale available in vacuum Maxwell theory; QED nonlinearity short by a factor of 2e34 at real intergalactic energy densities.
- **A supermassive-horizon-formation theorem, not previously in this project.** Horizon-forming density falls as 1/M^2, so above ~20,000 M☉ a horizon forms before any finite density cap is ever reached -- the cap is structurally never consulted at that mass (Sgr A* forms its horizon 50,000x below the stated cap; M87* at roughly the density of air). Rhoades & Ruffini (1974) independently blocks "use a stiffer exotic phase instead" for any equation of state, via causality + GR alone. This rules out "no horizon at all" for supermassive black holes specifically; it does not touch the separate question of finite structure behind a horizon once one forms.
- **An unreconciled internal contradiction, flagged not resolved.** Domain HH's cavitation picture requires a galaxy to be *lower* density than its surroundings; Domain CC's own refraction measurement (n increasing inward) shows the opposite. Both are this project's own prior work.
- **The R_core=30.73 km / rho_max circularity question already flagged in this project's own §13-equivalent status tracking is independently corroborated, not solved**, by the external material -- no script or document read confirms whether R_core is independently sourced or assumed.
- **A second, unreconciled flat-rotation-curve derivation** (2D vortex model, v=sqrt(k/m), independent of radius) exists in the external material and has never been checked against the SPARC-tested choke model this project actually uses.
- **Source-material quality flags, kept visible rather than smoothed over:** one external document (`Deepthink results .pdf`) contains an internal numerical inconsistency (the same nucleogenesis energy cost given as both ~1.8e61 erg and ~1.8e64 erg in different parts of the same file); `aistudiochat.pdf` was empty (share links only); the back two-thirds of `google perplecxity chat.pdf` and the later half of `final hypothesis1.pdf` were markedly less rigorous than their earlier sections and are flagged as such, not represented as equally weighted physics content.
