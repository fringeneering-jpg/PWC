...
for _, g in mt.iterrows():
    ...
    Sig = 0.5 * 10 ** (0.4 * (5.92 + 21.572 - mu)) * 1e6        # Msun/kpc^2
    ...
    for Ri, Vi in zip(b.R.values, b.V.values):
        gr = ring_gr(Ri, Rs, mring)
        gin = -gr[Rs < Ri].sum(); gout = gr[Rs > Ri].sum(); gnet = gin - gout
...
df.to_csv("probes/probes_accel.csv", index=False)
