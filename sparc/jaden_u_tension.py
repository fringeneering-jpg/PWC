"""U-shaped tension test (prediction frozen in predictions/u_tension.md).

g_pred = g_bar + sqrt(a0*g_bar) * [1 + s * |g_in - g_out|/(g_in + g_out)]
...
data["tight"] = np.abs(data.gin - data.gout) / (data.gin + data.gout)
data["tight_old"] = np.minimum(data.gin, data.gout) / data.gin

def shared(df, a0, s): return df.g_bar.values + np.sqrt(a0 * df.g_bar.values) * (1 + s * df.tight.values)
def old(df, a0, s): return df.g_bar.values + np.sqrt(a0 * df.g_bar.values) * (1 + s * df.tight_old.values)
...
a_s, s_all = fit2(data); a_o, s_o = fit2(data, old); a_b = fit1(data, base); a_r = fit1(data, rar)
print(f"FULL SAMPLE: s = {s_all:+.4f}  (a0={a_s:.4e})")
print(f"   shared-tension {galm(data, lambda d: shared(d, a_s, s_all)):.5f} | static shared {galm(data, lambda d: old(d, a_o, s_o)):.5f} | base {galm(data, lambda d: base(d, a_b)):.5f} | McGaugh {galm(data, lambda d: rar(d, a_r)):.5f}")
...
for sd in [7, 1, 2, 3, 4, 5, 6, 8, 9, 10]:
    ...
    a, s = fit2(tr); ao, so = fit2(tr, old); ab = fit1(tr, base); ar = fit1(tr, rar)
    R.append([sd, s, galm(ho, lambda d: shared(d, a, s)), galm(ho, lambda d: base(d, ab)), galm(ho, lambda d: rar(d, ar)), galm(ho, lambda d: old(d, ao, so))])
R = np.array(R)
print(f"\nHOLDOUT: s per split = {', '.join(f'{x:+.3f}' for x in R[:,1])}")
print(f"   mean: shared-tension {R[:,2].mean():.4f} | base {R[:,3].mean():.4f} | McGaugh {R[:,4].mean():.4f} | static shared {R[:,5].mean():.4f}")
print(f"   U beats static shared {int((R[:,2]<R[:,5]).sum())}/10")
...
rows = [dict(name=n.strip(), cls=lab.get(ty.get(n.strip())), S=pts(d, shared(d, a_s, s_all)), O=pts(d, old(d, a_o, s_o)), B=pts(d, base(d, a_b)), M=pts(d, rar(d, a_r))) for n, d in data.groupby(mc)]
df = pd.DataFrame(rows)
print("\nBY TYPE (full sample):")
for c, g in df.groupby("cls"): print(f"   {c:22s} n={len(g):>3}  U {g.S.mean():.4f}  static {g.O.mean():.4f}  base {g.B.mean():.4f}  McGaugh {g.M.mean():.4f}  U beats McGaugh {int((g.S<g.M).sum())}/{len(g)}  U beats static {int((g.S<g.O).sum())}/{len(g)}")
sp = df[df.cls.isin(["early spiral Sa-Sb","late spiral Sbc-Sd"])]
print(f"   SPIRALS combined n={len(sp)}: U {sp.S.mean():.4f}  static {sp.O.mean():.4f}  McGaugh {sp.M.mean():.4f}")
print("\n15 galaxies McGaugh fits worst:")
for _, r in df.sort_values("M", ascending=False).head(15).iterrows():
    print(f"   {r['name']:10s} {r['cls']:22s} McGaugh {r.M:.3f}  U {r.S:.3f}  static {r.O:.3f}  base {r.B:.3f}")
json.dump(dict(s_full=s_all, a0_full=a_s, s_static=s_o, a0_static=a_o, holdout=R.tolist()), open("jaden_u_tension_results.json", "w"), indent=1)
