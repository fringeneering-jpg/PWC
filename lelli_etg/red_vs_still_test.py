...
for f in sorted(glob.glob("rm/*_rotmod.dat")):
    ...
    vb2 = 0.7 * Vb * np.abs(Vb); vbar2 = Vg * np.abs(Vg) + 0.5 * Vd * np.abs(Vd) + vb2
...
t = pd.DataFrame(rows); pd.set_option("display.width", 160); print(t.round(3).to_string(index=False))
sp = t[~t.slow]
...
t.to_csv("red_vs_still_results.csv", index=False)
