...
- GWTC-4.0 blind (84 events, 2026-09-25) — NOTE: catalogue final masses come from GR remnant formulas, so this compares PWC with GR's model, not with an independent measurement: merger rule −0.56% ± 1.62%, but the residual trend is the rule's radiated fraction ∝ M^(−1/3) (6.3% → 3.8% across mass) against data rising slightly (3.6% → 5.2%). Either derive a mass-scaling for M_grad that stays near-scale-free, or show the regime picture beats this simpler reading — NEEDS DERIVATION
- **~47 M☉ crossover (prediction):** the yield radius √(GM/3.23×10¹¹) equals the choke 2GM/c² at M = c⁴/(4G·3.23×10¹¹) ≈ 47 M☉ (uses only the yield law and the choke). Below it the max-P boundary sits outside the acoustic horizon — light black holes (incl. the 3–5 M☉ mass gap and the 5–47 M☉ stellar range) would have exposed phase boundaries, not clean acoustic horizons like heavy/supermassive ones. **Existing data to face:** quiescent black-hole X-ray binaries (~5–20 M☉, e.g. Cyg X-1, A0620−00) are ~100× dimmer than neutron-star binaries at the same accretion rate — the standard evidence that infalling matter meets no surface (Narayan, Garcia & McClintock 1997; McClintock et al. 2004). An exposed shell must not re-radiate accreted energy, or it must be explained why not — NEEDS TEST
- **Two chokes (2026-09-25):** light and gravitational waves (transverse, c₀) are trapped inside 2GM/c² — the event horizon, where the §8 Hawking derivation applies unchanged. Compression (longitudinal) travels at c₀/√3 in the ordered medium and rises toward c₀ as the medium locks, so the compression choke for infalling *medium* sits between 2GM/c² and 6GM/c² (6GM/c² = GR's innermost stable circular orbit), set by the EOS locking rate. If at 6GM/c², the yield/choke crossover drops from ~47 M☉ to c⁴/(36G·3.23×10¹¹) ≈ 5 M☉ — near the lower edge of observed stellar black holes (the 3–5 M☉ mass gap; coincidence until derived) — NEEDS DERIVATION
- **Only the medium chokes; matter is caught and shredded (Jaden):** infalling matter is not a sound wave in the medium — it is caught at the event horizon and shredded (knots broken at ρ_max, the medium cycle). Shredding is an untying whose energy becomes medium (see recycling-bin item) — consistent with quiescent black-hole X-ray binaries showing no surface emission. v = √(2GM/r) is energy conservation outside the mass and is used only down to the choke, never continued to r → 0 — NEEDS DERIVATION
- **Black holes are a recycling bin, not a waste disposal (Jaden):** infalling matter gets stuck at max P (its light redshifts and fades from view) and is shredded — an untying, the vapour-forming side of cavitation, which draws heat in and does not glow (cavitation bubble growth absorbs latent heat; the flash — sonoluminescence — comes only on collapse). Energy is conserved: the energy the stuck matter loses (mc²·(1 − √(1 − r_s/R)) as counted far away) becomes medium — all of it — since the medium *is* energy (paired EM waves, m = L/c²). Neutron stars glow because gas hits a surface and stops (friction/compression). Quiescent black-hole X-ray binaries are ~100× fainter than neutron-star ones (Narayan, Garcia & McClintock 1997; Garcia et al. 2001); the faint glow they do show comes from the accretion flow outside, not from the shredding. Replaces "shredding must happen behind the light horizon." Derive: shredding → paired medium with no light escaping; entropy carried by the new medium's volume — NEEDS DERIVATION
- Thermodynamic stability of a PWC black hole with r_yield > r_sonic: does the exposed shell radiate/untie too fast to be stable? What does it do with accreted energy? — NEEDS DERIVATION
- **S-stars around Sgr A* — PASS (medium pull):** GRAVITY/Keck S2 orbit (pericentre ~120 AU) bounds any extended mass inside it to ≲ 0.1% of Sgr A* (~4,000 M☉; GRAVITY 2020). PWC's galaxy-scale medium term √(a₀·g_bar) with locked a₀ = 6.68×10⁻¹¹ adds ~6×10⁻⁶ of the pull at S2 (~25 M☉ equivalent) — far inside the bound. The cascade only matters where g → a₀: for Sgr A* alone at r = √(GM/a₀) ≈ 94 pc. PASSED
- Constraint on the BH profile from the same data: Sgr A*'s k-rule held medium (~23,000 M☉) must lie inside ~120 AU (S2 pericentre) — NEEDS DERIVATION (radial profile)
- **k-rule = universal held column density:** M_med = 0.869·C^(2/3) ⇔ Σ = M_med/4πR_c² = 8.48×10²⁰ kg/m² for every core (≈3.7 km of nuclear-density skin, or ≈650 km at ρ_max). Hydrostatic atmospheres fail (cores >~9 M☉ lie inside their own choke — medium there cannot be static); a steady-inflow model was also tried and withdrawn — PWC's medium does not inflow (stationary medium; a set black hole does not keep drawing medium in). The held medium is static and tension-held, built from the medium itself (Jaden): the black hole holds the surrounding medium in tension and compression, as every mass holds the medium within its reach. Matter carried in by gravity is trapped and shredded into medium, and that medium joins the medium and grows as the medium always does — invisibly, apart from a small amount of heat released by friction. Read this way the k-rule is a surface-capacity law: each core's surface holds at most Σ per m²; in a merger the combined core has less surface per unit mass, so Σ × (A₁ + A₂ − A_f) is released — exactly the k-rule's release. **Candidate law for Σ (2026-09-25): the held medium stacks until its own self-pull reaches the holding yield, 2πGΣ = a_yield → Σ = a_yield/(2πG) = 7.70×10²⁰ kg/m² (a_yield = 3.23×10¹¹) vs the k-rule's 8.48×10²⁰ (−9%).** Status: candidate, found by checking combinations after Σ was known. Open issues: (i) a_yield and ρ_max both come from GW150914 (the √(40/3)·c₀√(Gρ_max) match is a numerical match, not a derivation — see DERIVATION_BRIEF Round 1; c₀√(4πGρ_max) = 3.135×10¹¹ gives Σ = 7.48×10²⁰, −12%); (ii) the gap does not close inside nuclear-density uncertainty — k-rule Σ ∝ ρ_nuc^(2/3) rises with ρ_nuc (9.67×10²⁰ at 2.8×10¹⁷); matching needs ρ_nuc ≈ 1.99×10¹⁷, just below the 2.3–2.8×10¹⁷ range. What sets Σ is the open part — and it must include self-holding: the medium has mass, so a thick max-P layer adds to the pull holding the medium outside it (the cascade dM_med/dr = dM_bar/dr + M_med/r at black-hole scale), strongest for small cores where the medium share is largest (Jaden). Decisive data: a loud light merger's ringdown-measured final mass (PWC predicts a larger release than GR there). The choke is read statically: the radius where escape speed √(2GM/r) reaches c₀ — nothing can climb back out — not a place medium flows through. Derive what fixes a universal column Σ at the core surface — NEEDS DERIVATION
- Black-hole radial profile (Jaden: the medium thickens gradually the whole way in — v = √(2GM/r) assumes empty space): infall must use v(r)² = 2∫G·M_enc(r′)/r′² dr′ with M_enc including the medium, against a sound speed c_s(r) rising from c₀/√3 toward c₀ as the medium locks; the choke is where they meet. All choke radii quoted 2026-09-25 (44 km, 47 M☉, 2GM/c² for compression) used standard empty-space infall — they are the standard (space-weighs-nothing) values, not PWC values; PWC's come from the medium profile. Derive ρ(r), P(r), c_s(r) and the stress law from core → max-P → decompression (thicker, not denser), then PWC's own perturbation equation for ringdown modes. Until then the k-rule's M_grad is apparent-mass bookkeeping, not mass at definite radii; this also applies to §3's ρ_max = M_grad/shell-volume. (An assumed-profile estimate, `predictions/gwtc4_ringdown_mass.md`, was withdrawn as a PWC prediction.) Known geometric fact independent of M_grad placement: the yield radius √(GM/3.23×10¹¹) exceeds the choke 2GM/c² below ~47 M☉ — NEEDS DERIVATION
...
- **Redshift = reordering, everywhere (Jaden):** one mechanism for (1) cosmological redshift — light crossing space reorders into paired medium (§7); (2) gravitational redshift — light climbing out of a well reorders as it climbs; (3) black-hole darkness — at max P nearly all of it reorders before escaping, and its energy becomes medium. Must reproduce the measured gravitational redshift exactly, Δν/ν = gh/c² (Pound–Rebka 1960: 2.5×10⁻¹⁵ over 22.5 m; Gravity Probe A 1976 and Galileo 2018: agreement to ~10⁻⁴–10⁻⁵), from the same medium loading that bends light by the factor 2 (T11) — NEEDS DERIVATION
