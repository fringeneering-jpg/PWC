"""GWTC-4.0 blind tests (predictions frozen in predictions/gwtc4_blind.md, commit c22fecc).
...
rows = []
for v in d.values():
    if v["catalog.shortName"] != "GWTC-4.0": continue
...
print(f"GWTC-4.0 BBH events: {len(t)} (all O4a, none in the earlier 89)  names: {t.event.str[:4].unique()}")
...
t.to_csv("gwtc4_blind_results.csv", index=False)
