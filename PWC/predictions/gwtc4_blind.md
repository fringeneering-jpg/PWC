...
## Caveat added 2026-09-25 (applies to Test A and its trend)
GWTC final masses are not independent measurements: the inspiral masses and spins are fitted with GR waveform models and the final mass is computed from them with GR numerical-relativity remnant formulas. Test A therefore shows the frozen k-rule reproducing GR's remnant formula to −0.56% ± 1.62% on 84 events. The size trend (catalogue radiated fraction rising slightly with mass, rule falling; not a mass-ratio artifact — corr(q, M_total) = −0.05, trend persists at fixed q) is PWC vs GR's scale-free-radiation assumption, not PWC vs an independent measurement. Only ringdown-measured final masses (IMR consistency) test it independently.
