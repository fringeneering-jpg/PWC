...
**Two different waves, one medium.** Water carries sound (compression, ~1,500 m/s) and surface waves (neighbour reaction) at very different speeds. The medium does the same: light and gravitational waves are the neighbour-reaction mode at c₀; compression is a separate, slower mode. In the ordered resting medium — a 3D radiation-like EM lattice, P = ε/3 — compression travels at c₀/√3; in the locked state at ρ_max — a 1D stiff lattice, P = ε — it reaches c₀ (the sonic choke, §8).
