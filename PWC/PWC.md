...
- A hard maximum. The medium packs up to a ceiling density and stops. A larger core does not make it denser — it makes the maximum-density layer **thicker**, out to the radius where that core's pull has weakened below what the ceiling needs; only then does the medium decompress. The medium's own mass is included in the 1/r² profile.
...
**Deriving the √(a₀·g_bar) term from the tension rule.** Tension *is* the pull: a mass tensions the medium it pulls on, which is why 1/r² works outside a mass — a vacuum has no reason to follow 1/r², the medium carrying the pull does. Near the mass the pull tensions the medium in 3D, g = GM/r². At the radius r_t where the pull falls to the holding threshold a₀ — too weak to tension resting medium any further — GM/r_t² = a₀, so r_t = √(GM/a₀) (the same M^½ holding reach as Step 3). Beyond r_t the pull carries on only along the tensioned disk, spreading over circles instead of spheres, so it falls as 1/r from a₀ at r_t: g = a₀·r_t/r = √(GM·a₀)/r = **√(a₀·g_bar)** — exactly the extra term in Step 7's formula. **a₀ is the holding threshold for resting medium**, the galaxy-scale counterpart of the 3.23×10¹¹ N/kg threshold at ρ_max.
...
- **Growth concentrates at the thickest walls of the cosmic web** — the dense rings around voids, especially at the pronounced borders between large galaxies. Walls growing outward on both sides push into the neighbouring voids: this is what drives void expansion. The thickest rings sit at a preferred spacing — the BAO scale, ~147 Mpc — read in PWC as the shells where a sound wave in the medium, launched at the formation event (the great freezing), froze in place.
...
**Why light moves at exactly c₀, and not some other speed:** light's speed isn't set by how hard any particular squeeze event is — it's a fixed property of the medium itself. Light is not a compression wave. Like a wave on the ocean — which is not the water compressing, just each parcel reacting to the one next to it — light is a **transverse neighbour-reaction wave**: each paired wave in the medium is yanked by its neighbours' electromagnetic pull and push, and passes the reaction on. Its speed is set by that EM restoring yank against the medium's inertia, √(restoring stiffness / inertia), the same form as a wave on a string. For the vacuum this is the measured

    c₀ = 1/√(μ₀ε₀)

with 1/ε₀ the medium's electric springiness (how hard it yanks back) and μ₀ its inertia. Maxwell found light exactly this way (*On Physical Lines of Force*, 1861–62): he modelled the vacuum as a real medium with elasticity and density, computed its wave speed, and it came out equal to the measured speed of light. That's why c₀ is the same everywhere the medium's pairing is the same, whatever squeezed a given photon out. **Gravitational waves are the same neighbour-reaction wave of the same medium**, hence the same speed (GW170817: equal to 1 part in 10¹⁵).

**Two different waves, one medium.** Water carries sound (compression, ~1,500 m/s) and surface waves (neighbour reaction) at very different speeds. The medium does the same: light and gravitational waves are the transverse neighbour-reaction mode at c₀; compression is the separate, slower longitudinal mode. In the ordered resting medium — a 3D radiation-like EM lattice, P = ε/3 — compression travels at c₀/√3; in the locked state at ρ_max — a 1D stiff lattice, P = ε — it reaches c₀ (the sonic choke, §8).
...
### Single-source run (2026-09-21): all five mergers within 2.2%
...
Every event within 2.2%, spanning q=0.33 to q=0.86 and total mass ~15 to ~66 M☉ — no q-dependent or spin-dependent pattern visible at all.
...
| PROBES 331 with ALFALFA HI gas added | locked from SPARC | 0.2452 | 0.2490 | 0.2511 |

On PROBES (1342 galaxies, no gas, no bulge split) the model beats McGaugh by 1.7% (bootstrap 95% range 1.3–2.1%, excludes zero) and base in 988/1342 galaxies, with zero constants fitted to PROBES. Adding ALFALFA gas to 331 of them did not erode the lead (vs McGaugh 1.8% → 2.3%; vs base 1.5% unchanged), so the tension term is not standing in for missing gas; at M/L 0.6 with gas it ties McGaugh and still beats base. Survived, not proven. On GHASP the tension term itself is visible: switching it off (s = 0, same a₀) gives 0.2695; it helps in 52/81 galaxies. On the dwarfs it barely switches on (little outer mass to share with). The LITTLE THINGS and GHASP data are noisier than SPARC (rebuilt baryons; Hα only, no gas, optical-colour M/L), and all margins are under ~3%.
