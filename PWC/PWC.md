...
The Phase Wave Cosmology framework: rules, mechanisms, derivations and results. The core derivation chain is §0. Hypotheses that were tested and dropped are logged in `TESTED_AND_DROPPED.md`; outside review and open questions are kept separately in `REVIEW.md`.
...
**The quicycle knot (from Williamson's toroidal photon).** Matter is a 720° phase-locked topological knot of light. The loop's chirality sets the charge sign — clockwise and counter-clockwise locks of the same photon give matter and antimatter. Matter is a sieve, not a solid: ~99.9% empty, so it passes through the medium without drag and interacts by configuration. One substance, four states, all governed by the same c₀² threshold (m = L/c₀²): **locked** (mass), **ordered** (the resting medium, paired positive/negative), **sheared** (charge — what a magnet does to ordered medium in induction), and **radiated** (light).
...
Heat in the medium does not simply diffuse at random — in a rotating region it is channelled. The medium is chiral (the paired EM(+)/EM(−) birefringent structure of §1, and the handed quicycle knot of §5), and in a rotating chiral medium heat propagates as a **coherent wave along the rotation axis**. This is established physics in other systems: the **chiral heat wave**, driven by the mixed gauge-gravitational anomaly of quantum field theory (Chernodub et al., ~2015 onward), worked out for rotating quark-gluon plasma and Weyl semimetals; in magnetic fields or dense matter it mixes with the chiral magnetic and chiral vortical waves. Related directed heat transport is seen in **chiral phonons** (lattice vibrations carrying heat and angular momentum together, used for magnetically switched nanoscale thermal devices) and in **non-Hermitian thermal systems**, where heat moves with directional robustness near exceptional points.

In PWC this is why heat moves to the untying events along coherent paths rather than spreading evenly: rotation plus the medium's chirality gives heat a preferred direction. It gives the thermal web a directed-transport mechanism, and ties naturally to structures that already emit along their spin axis — black-hole jets and the rotation axes of galaxies.
...
- A Q-transform ridge trace `f_ridge(t)` — the loudest time-frequency track — extracted with a **MAD-based robust noise floor**, not a hand-tuned frequency-sweep filter (see REVIEW.md on why that distinction matters).
